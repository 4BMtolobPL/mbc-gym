...
def make_color(labels: List[str]) -> List[int]:
    colors: List[List[int]] = [
        [random.randint(0, 255) for _ in range(3)] for _ in labels
    ]
    color: List[int] = random.choice(colors)
    return color
...
def exec_detect(target_image_path: Path) -> Tuple[List[str], str]:
    labels: List[str] = current_app.config["LABELS"]
    image = Image.open(target_image_path)
    image_tensor = torchvision.transforms.functional.to_tensor(image)
    model = torch.load(
        Path(current_app.root_path).parent / "models" / "model.pt", weights_only=False
    )
    model.eval()
...
@detector_views.get("/images/search")
def search():
